...
- 新增开机即玩的小学口算游戏:上电即出一道三位数以内的 `+`、`-`、`x`、`÷` 题并给出四个选项,上/下移动高亮、确定提交;答对显示 `CORRECT` 并等待任意按键,答错显示 `TRY AGAIN` 可重新选择,再次答错则揭示正确答案并等待按键进入下一题,如此无限循环。出题与“两次机会”状态机在带主机测试的 `main/math_quiz.c`,`main/demo_math.c` 在共用的 `ui_pixel` 主题上渲染,右上角显示电量,除号用像素方块手绘(内置字库没有 `÷`/`×` 字形);长按“确定”仍可返回演示菜单。
